...
> `state.activeField` (`ActiveField.originalPrice` / `discountRate` / `extraDiscountRate`)에 따라 키패드 입력 대상이 달라진다. ViewModel(`DiscountCalculatorViewModel`)이 관리.
...
> 필드 탭 시 `state.activeField = ActiveField.originalPrice`로 전환.
...
> 할인율은 0–99 범위 (정수만). 칩 선택 시 해당 값으로 직접 입력 필드 동기화.
> 필드 탭 시 `state.activeField = ActiveField.discountRate`로 전환.
...
> 추가 할인 토글 시 `state.activeField = ActiveField.extraDiscountRate`로 자동 전환.
> 추가 할인 제거 시 `state.extraRate` 초기화 후 `state.activeField = ActiveField.discountRate`로 복귀.
...
| ❺ | "할인 금액" 레이블 | `Text` | `textStyleBody` 16sp / w400 | `kDiscountTextSecondary` |
| ❺ | 할인 금액 수치 | `Text` | `textStyleBody` 16sp / w600 | `kDiscountTextSavings` |
...
> 금액이 표시되는 모든 위치(원가 취소선·최종가 행·할인 금액·최종가 대형)에 `_getCurrencySymbol()` 반환값이 앞에 붙는다. 기호 없는 국가는 숫자만 표시.
...
| `kDiscountTextSavings` | `#FFB8D6` | 할인 금액 강조 텍스트 |
