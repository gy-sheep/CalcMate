...
│  │  환율 기준 통화       KRW › │  │  → 바텀시트 선택 (UI only)
│  │  BMI 단위          kg/cm › │  │  → 바텀시트 선택 (UI only)
│  └─────────────────────────────┘  │
│                                   │
│  ┌─────────────────────────────┐  │
│  │  앱 정보                    │  │  섹션 타이틀 (w700)
...
- **현재 UI 타일만 구현** — URL 및 `url_launcher` 연동 미구현
...
- **개인정보 처리방침 URL**: URL 확정 후 `url_launcher` 연동
