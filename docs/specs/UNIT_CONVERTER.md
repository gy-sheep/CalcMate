...
- 카테고리: **길이**, **질량**, **온도**, **넓이**, **시간**, **부피**, **속도**, **연비**, **데이터**, **압력**
- 선택된 카테고리는 하이라이트(배경색 또는 밑줄)로 구분한다
- 카테고리 변경 시 활성 단위는 해당 카테고리의 기본 단위로, 입력값은 `0`으로 초기화한다

### 스와이프 카테고리 전환

- 단위 리스트 영역을 좌우 스와이프하면 이전/다음 카테고리로 전환된다
- 카테고리 탭과 양방향 연동된다 (스와이프 → 탭 이동, 탭 선택 → 페이지 이동)
- 키패드 영역은 스와이프 대상에서 제외한다 (키패드 버튼 터치와 충돌 방지)
...
- **`00`**: 온도를 제외한 모든 카테고리에서 표시. 선행 0 방지 (입력이 `0`이면 무시), 자릿수 제한에 따라 0을 1개 또는 2개 추가
- **`+/-`**: 온도 카테고리에서만 `00` 대신 표시. 부호 전환 (양수 ↔ 음수)
...
| 조건 | 표시 형식 |
|------|-----------|
| 1e15 이상 | 지수 표기법 (예: `1.23e+15`) |
| 1,000,000 이상 1e15 미만 | 소수점 없이 정수 표시 + 천 단위 콤마 (예: `1,234,567`) |
...
- **온도 카테고리**: 소수점 최대 3자리로 제한한다 (예: `273.15`, `-40`)
- **활성 단위 전환 시**: 변환 결과를 새 입력값으로 설정하되, 첫 키 입력 시 기존 값을 대체한다 (isResult 패턴)
