...
## 2026-03-13 — 기본 계산기 동작 개선 및 설정 UI 서브 섹션 추가

### 완료 항목

**기본 계산기**
- 연산자 뒤 음수 대기(`5×-`) 제거 → 연산자 교체로 통일
- 결과 상태에서 괄호 → `결과×(` 연결 (기존: `(` 새 수식)
- `(` 뒤 `%` → 무시 (기존: `0%` 삽입)
- `(` 로 끝나면 `=` 무시 (빈 괄호 방지)
- `lastTopLevelSegment` 도입: 반복 `=` 에서 괄호 포함 수식 올바르게 대응
- 테스트 업데이트 (6건)

**환율 계산기**
- `deviceCurrencyProvider` 추가: 기기 통화 기반 기본 toCodes 구성 (baseCurrency·기기 통화 중복 제거)

**단위 변환기**
- 결과 상태에서 백스페이스 → 0 리셋 대신 마지막 문자 삭제로 변경

**설정 화면**
- "일반" 섹션에 서브 섹션 헤더 추가 (메인 리스트 / 환율 계산기)
- `_SubSectionHeader` 위젯: `surfaceContainerHighest` 40% 배경 tint + 8px 라운드
- l10n 문자열 추가: `settings_subMain`, `settings_subCurrency` (ko/en)
